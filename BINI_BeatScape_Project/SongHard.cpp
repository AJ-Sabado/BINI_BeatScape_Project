...
namespace BINI
{
	...
	SongHard::SongHard(BINI::Renderer* renderer)
	{
		//Textures
	...
	void SongHard::display(BINI::Renderer* renderer)
	{
		//Beat pulse
	...
	bool SongHard::handleEvents(BINI::Events* events)
	{
		if (beat > 872 || accuracy < 65.f)
		{
			if (sceneAlpha == 255)
			{
				...
				//Store user results
				events->setUserData(score, maxCombo, "Hard", accuracy);
	...
	Uint8 SongHard::barHighlightsfadeOut(Uint8 highlight)
	{
		if (highlight - 10 < 0)
		{
			return 0;
